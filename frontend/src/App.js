...
/* ─────────────────────────────────────────────────────────────────
   AUTH PAGES (SignIn, SignUp, VerifyEmail, Forgot, Reset)
  ...
  return (
    ...
    >
      <div style={{
        ...
        background: checked ? "#fff" : "#555", transition: "left 0.2s",
      }}/>
    </button>
  );
}
...
function ProfileModal({ onClose }) {
  ...
  return (
    <div style={S.overlay} onClick={onClose}>
      <div style={{ ...S.modal, width: 580, padding: 0, maxHeight: "85vh", overflow: "hidden", display: "flex", flexDirection: "column" }} onClick={e=>e.stopPropagation()}>
        {/* Header */}
          ...
          {tab === "profile" && (
            <div>
                ...
                <div style={{ position:"relative" }}>
                  {avatarSrc ? (
                    <img src={avatarSrc} alt="" referrerPolicy="no-referrer"
                      ...
                      style={{ width:72, height:72, borderRadius:14, objectFit:"cover", border:"2px solid #1e1e1e" }}/>
                  ) : (
                    <div style={{ width:72, height:72, borderRadius:14, background:"#1a1a1a", border:"2px solid #1e1e1e", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, fontWeight:700, color:"#666" }}>
                      {initials}
                    </div>
                  )}
                  <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
                    style={{ position:"absolute", bottom:-4, right:-4, width:26, height:26, borderRadius:"50%", background:"#3b82f6", border:"2px solid #0f0f0f", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", opacity:uploading?0.5:1 }}>
                    {uploading ? <div style={{ width:10,height:10,border:"2px solid rgba(255,255,255,0.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin 0.6s linear infinite" }}/> : <FaCamera size={10}/>}
                  </button>
                  <input ref={fileInputRef} type="file" accept="image/*" style={{ display:"none" }} onChange={uploadAvatar}/>
                </div>
                <div>
                  <div style={{ fontSize:16, fontWeight:600, color:"#e5e5e5", marginBottom:3 }}>{user?.name}</div>
                  <div style={{ fontSize:12, color:"#555", marginBottom:6 }}>{user?.email}</div>
                  <div style={{ fontSize:10, color:"#3a3a3a", textTransform:"capitalize", background:"#141414", display:"inline-block", padding:"2px 8px", borderRadius:4 }}>
              ...
              <button onClick={saveProfile} disabled={saving || !name.trim()}
                style={{ ...T.btn, height:40, background:"#3b82f6", color:"#fff", opacity:(!name.trim())?0.4:1 }}>
          ...
          {tab === "security" && (
            <div>
              ...
              <button onClick={changePassword} disabled={pwdSaving || !currentPwd || !newPwd || !confirmPwd}
                style={{ ...T.btn, height:40, background:"#3b82f6", color:"#fff", opacity:(!currentPwd || !newPwd || !confirmPwd)?0.4:1 }}>
...
function SettingsModal({ onClose, settings, setSettings }) {
  const [tab, setTab] = useState("editor"); // editor | appearance | notifications | about
  ...
  const Row = ({ label, hint, children }) => (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 0", borderBottom:"1px solid #141414" }}>
      <div style={{ flex:1 }}>
        <div style={{ fontSize:12, color:"#d4d4d4", fontWeight:500 }}>{label}</div>
  ...
  const Select = ({ value, onChange, options }) => (
    <select value={value} onChange={e => onChange(e.target.value)}
      style={{ background:"#080808", border:"1px solid #1e1e1e", borderRadius:6, color:"#d4d4d4", fontSize:12, padding:"6px 10px", outline:"none", fontFamily:"'Geist',sans-serif", cursor:"pointer", minWidth:120 }}>
  ...
  const NumInput = ({ value, onChange, min, max, step = 1 }) => (
    <input type="number" value={value} min={min} max={max} step={step}
      ...
      style={{ background:"#080808", border:"1px solid #1e1e1e", borderRadius:6, color:"#d4d4d4", fontSize:12, padding:"6px 10px", outline:"none", width:80, fontFamily:"'JetBrains Mono',monospace" }}/>
  );
    ...
    <div style={S.overlay} onClick={onClose}>
      <div style={{ ...S.modal, width: 600, padding: 0, maxHeight: "85vh", overflow: "hidden", display: "flex", flexDirection: "column" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ display:"flex", flex: 1, overflow: "hidden" }}>
          {/* Sidebar tabs */}
          <div style={{ width:170, borderRight:"1px solid #1a1a1a", padding:"12px 0", flexShrink: 0, background:"#0c0c0c" }}>
            {tabs.map(t => {
              const Icon = t.icon;
              return (
                <button key={t.id} onClick={() => setTab(t.id)} style={{
                  width:"100%", background: tab===t.id ? "#1a1a1a" : "transparent", border:"none",
                  padding:"10px 18px", color: tab===t.id ? "#e5e5e5" : "#666", fontSize:12,
                  fontWeight:500, display:"flex", alignItems:"center", gap:10,
                  cursor:"pointer", borderLeft: tab===t.id ? "2px solid #a78bfa" : "2px solid transparent",
                  fontFamily:"'Geist',sans-serif", textAlign:"left", transition:"all 0.15s",
                }}>
                  <Icon size={11}/>{t.label}
                </button>
              );
          ...
          <div style={{ flex:1, padding:"20px 24px", overflowY:"auto" }}>
            {tab === "editor" && (
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:"#3a3a3a", letterSpacing:1, textTransform:"uppercase", marginBottom:8 }}>Code Editor</div>
                ...
                <Row label="Word Wrap" hint="Wrap long lines in the editor">
                  <Select value={settings.wordWrap} onChange={v => update("wordWrap", v)}
                ...
                <Row label="Line Numbers" hint="Show line numbers in gutter">
                  <Select value={settings.lineNumbers} onChange={v => update("lineNumbers", v)}
                ...
                <Row label="Cursor Blinking" hint="Animation style for cursor">
                  <Select value={settings.cursorBlinking} onChange={v => update("cursorBlinking", v)}
                    options={[{value:"smooth",label:"Smooth"},{value:"blink",label:"Blink"},{value:"phase",label:"Phase"},{value:"expand",label:"Expand"},{value:"solid",label:"Solid"}]}/>
                </Row>
                ...
                <div style={{ fontSize:11, fontWeight:700, color:"#3a3a3a", letterSpacing:1, textTransform:"uppercase", marginTop:24, marginBottom:8 }}>Auto Save</div>
            ...
            {tab === "appearance" && (
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:"#3a3a3a", letterSpacing:1, textTransform:"uppercase", marginBottom:8 }}>Theme</div>

                <Row label="Editor Theme" hint="Color theme for the code editor">
                  <Select value={settings.theme} onChange={v => update("theme", v)}
                    options={[
                ...
                <div style={{ marginTop:20, padding:"14px", background:"#0a0a0a", border:"1px solid #1a1a1a", borderRadius:8 }}>
                  <div style={{ fontSize:11, color:"#666", lineHeight:1.7 }}>
                    <FaInfoCircle size={10} style={{ display:"inline", marginRight:6, color:"#3b82f6" }}/>
            ...
            {tab === "notifications" && (
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:"#3a3a3a", letterSpacing:1, textTransform:"uppercase", marginBottom:8 }}>Preferences</div>
            ...
            {tab === "about" && (
              <div>
                <div style={{ textAlign:"center", padding:"20px 0" }}>
                  <div style={{ width:64, height:64, borderRadius:14, background:"#1a1a1a", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 16px" }}>
                    <FaCode size={26} color="#e5e5e5"/>
                  </div>
                  <div style={{ fontSize:18, fontWeight:700, color:"#e5e5e5", marginBottom:4 }}>CloudIDE</div>
                ...
                <div style={{ background:"#0a0a0a", border:"1px solid #1a1a1a", borderRadius:8, padding:"14px" }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                    {[
                      ...
                      <a key={label} href={href} style={{ fontSize:12, color:"#3b82f6", textDecoration:"none" }}>→ {label}</a>
                    ))}
                  </div>
                </div>
        ...
        {/* Footer */}
        <div style={{ padding:"14px 24px", borderTop:"1px solid #1a1a1a", display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink: 0 }}>
          <button onClick={resetDefaults}
            style={{ background:"transparent", border:"1px solid #2a2a2a", borderRadius:6, padding:"6px 12px", fontSize:11, color:"#666", cursor:"pointer", fontFamily:"'Geist',sans-serif" }}>
            Reset to Defaults
          </button>
          <div style={{ fontSize:10, color:"#3a3a3a" }}>Settings save automatically</div>
        </div>
      </div>
    </div>
  );
...
function IDE() {
  ...
  }, [activeFile.code, activeFile.saved, settings.autoSave, settings.autoSaveDelay, saveFile]);
    ...
    <div style={S.root} onClick={() => { setUserMenu(false); setCtxMenu(null); }}>
      ...
      <div style={S.body}>
        ...
        <div style={S.actBar}>
          ...
          <div style={{flex:1}}/>
          <button style={S.actBtn} onClick={() => setSettingsModal(true)} title="Settings"><FaCog size={16}/></button>
        </div>
